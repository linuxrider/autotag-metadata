...
def test_write_metadata_json_format_writes_json(tmp_path):
    f = tmp_path / "data.csv"
    f.write_bytes(b"x")
    params = {"experiment": "test", "value": 42}
...
def test_write_metadata_format_independent_of_suffix(tmp_path):
    f = tmp_path / "data.csv"
    f.write_bytes(b"x")
...
def test_write_metadata_json_preserves_unicode_and_order(tmp_path):
    f = tmp_path / "data.csv"
    f.write_bytes(b"x")
